...
public class LottoSimulator {
	...
	public static int[] createLotto() {
		...
		int[] lotto = new int[6]; // 6개의 로또 숫자를 담을 배열을 생성
		 ...
		 */
		int i = 0; //인덱스(제어변수)
		while(i < 6) { // 번호를 6까지 뽑고 멈춘다.
			boolean flag = false; // 중복 여부를 검사하기 위한 변수
			int rn = r.nextInt(45) + 1; // 1부터 46까지의 로또 번호를 뽑는다.
			for(int j=0; j<=i; j++) { // 중복 여부를 판단하기 위한 로직, 뽑힌 rn을 j가 i번까지 검사한다.
				if(rn == lotto[j]) { // 만약 중복이라면
					flag = true; // 변수를 t로 바꾸고
					break; // for문 탈출 후 다시 번호를 뽑는다.
				}
			}
			if(!flag) { // 중복이 아니라면
				lotto[i] = rn; // i번째 인덱스에 로또번호를 넣는다.
				i++; // 반복문의 제어변수를 1씩 올린다.
			}
		}
		Arrays.sort(lotto);
