...
public class Player {
	...
	// 생성자
	Player() {
		System.out.println("플레이어의 기본 생성자 호출");
...
//		hp = 50;
		...
		// 매개변수와 지역변수의 이름이 똑같으면 값을 지목할 수 없기 때문에 
		// 맴버변수의 이름을 바꾸거나, this.맴버변수 로 사용해야 한다.
		// this는 지금 생성되고 있는 그 객체를 가리킨다.
	}
	Player(String name, int hp) {
		this(name); // 바로 위의 String name이 들어있는 2번 생성자를 호출! 여기서 name은 this가 가리키고 있는 객체(실시간으로 실행되는 객체)인 플레이어 3번의 이름을 가리킨다.
		System.out.println("플레이어의 3번 생성자 호출!");
//		this.name = name;
